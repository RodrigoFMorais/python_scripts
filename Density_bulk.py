###############################################################################
#       Code to calculate the density of bulk water at each time step         #
#       by Rodrigo Ferreira de Morais                                         #
#       Last modification date: 04/05/2015                                    #
###############################################################################
...
dPtO=3.6 #Pt-O maximum distance in A.
...
TotalVolume=vect[0][0]*vect[1][1]*vect[2][2]
Amo=2.67E-23
Amh=1.67E-24
DH2O=[]
...
for nf in range(1,type+1):
	...
	for li in range(0,inter):
		...
		temp=CalcDist(AtomOCoordC[LPhiH2O[0]],CM)
		HPH2O=0
		for i in range(1,len(LPhiH2O)):
			dist=CalcDist(AtomOCoordC[LPhiH2O[i]],CM)
			if dist>temp:
			 	HPH2O=i
				temp=dist
		...
		HPPt=0
		temp1=CalcDist(AtomPtCoordC[0],CM)
		for i in range(1,len(AtomPtCoordC)):
			dist=CalcDist(AtomPtCoordC[i],CM)	
			if dist>temp1:
				HPPt=i
				temp1=dist
		...
		RVolume=(TotalVolume - (4/3)*3.14*(temp*temp*temp))*1E-24
                TVolume=(TotalVolume - (4/3)*3.14*(temp1*temp1*temp1))*1E-24

		TO=len(AtomOCoordC)-len(LPhiH2O)

		MT=((TO*Amo)+((TO*2)*Amh))
		MT1=((len(AtomOCoordC)*Amo)+((len(AtomOCoordC)*2)*Amh))

		xxx=[]
		xxx.append(RVolume)
		xxx.append(TO)
		xxx.append(MT)
		xxx.append(MT/RVolume)
		xxx.append(temp)
		xxx.append(temp1)
		xxx.append(MT1/TVolume)
		DH2O.append(xxx)
...
for i in range(0,len(DH2O)):
	print i,DH2O[i][0],DH2O[i][1],DH2O[i][2],DH2O[i][3],DH2O[i][4],DH2O[i][5],DH2O[i][6]
